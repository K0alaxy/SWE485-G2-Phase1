# SWE485 Project: Employee Attrition Prediction System

## 1. Group Information
### Group 2
### Members
- Lana – 444200666
- Najla - 444200597
- Nouf Alkathiri – 444200698
- Jood Alshehri - 444200773
- Fatema Makled - 444200114
- Nouf - 444200408
- Nada - 444200546

---

## 2. Project Title and Motivation
### **Predictive Analysis of Employee Attrition**

Employee attrition represents a critical challenge for organizations, as companies often struggle to identify the specific factors that lead employees to leave. With a large workforce, the challenge lies in using employee data to uncover hidden patterns that indicate whether an employee is likely to leave the company. 

To address this, we are using the IBM HR Employee Attrition dataset, a structured tabular dataset containing 1,470 records and 35 features. This data—covering demographics, job roles, and satisfaction levels—will allow us to build a binary classification system to predict the target variable "Attrition" (Yes/No).







